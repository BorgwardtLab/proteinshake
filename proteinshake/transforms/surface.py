...
class SurfaceTransform(ShakeTransform):
    ...
    def __call__(self, protein):
        ...
        new_prot["residue"] = self._surf_filter(
            protein, surf, resolution="residue"
        )
        if mode == "atom":
            new_prot["atom"] = self._surf_filter(
                protein, surf, resolution="atom"
            )
        return new_prot
    ...
    def _surf_filter(self, protein, surface_df, resolution="residue"):
        ...
        new_data = {
            ...
            "residue_type": [
                AA_THREE_TO_ONE[a] for a in surface_df["residue_name"].tolist()
            ],
