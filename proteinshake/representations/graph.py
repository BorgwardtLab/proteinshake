...
class Graph:
    ...
    def __init__(self, protein, construction, k, eps, weighted_edges):
        ...
        nodes = tokenize(
            protein[resolution][f"{resolution}_type"], resolution=resolution
        )
        if construction == "eps":
...
class GraphDataset:
    ...
    def __init__(
        ...
        assert not (
            eps is None and k is None
        ), "You must specify eps or k in the graph construction."
        ...
        self.graphs = (
            Graph(protein, construction, k, eps, weighted_edges)
    ...
    def pyg(self, *args, **kwargs):
        ...
        return PygGraphDataset(
            self.graphs, self.size, self.path + ".pyg", *args, **kwargs
        )
    ...
    def dgl(self, *args, **kwargs):
        ...
        return DGLGraphDataset(
            self.graphs, self.size, self.path + ".dgl", *args, **kwargs
        )

    def nx(self, *args, **kwargs):
        from proteinshake.frameworks.nx import NetworkxGraphDataset

        return NetworkxGraphDataset(
            self.graphs, self.size, self.path + ".nx", *args, **kwargs
        )
