...
class Voxel:
    ...
    def __init__(self, protein, gridsize, voxelsize, aggregation):
        ...
        coords -= coords.min(
            axis=0
        )  # translate to make all coords positive and flushed to the axes
        voxel_indices = (coords / voxelsize).astype(np.int32)  # rasterize
        voxels = np.zeros(
            shape=np.concatenate([voxel_indices.max(axis=0) + 1, labels.shape])
        )
        counts = np.zeros(
            shape=np.concatenate([voxel_indices.max(axis=0) + 1, labels.shape])
        )
        voxel_indices = np.concatenate(
            [voxel_indices, np.expand_dims(np.arange(len(coords)), 1)], axis=1
        )
        ...
        upper = (
            voxels.shape[:-1] * (diff >= 0) + (diff < 0) * np.floor(diff / 2)
        ).astype(int)
        voxels = voxels[
        ...
        # pad to gridsize
        diff = gridsize - voxels.shape[:-1]
        paddings = np.stack([np.ceil(diff / 2), np.floor(diff / 2)], 1).astype(
            np.int32
        )
...
class VoxelDataset:
    ...
    def __init__(
        ...
        self.voxels = (
            Voxel(protein, gridsize, voxelsize, aggregation)
    ...
    def torch(self, *args, **kwargs):
        ...
        return TorchVoxelDataset(
            self.voxels, self.size, self.path + ".torch", *args, **kwargs
        )

    def tf(self, *args, **kwargs):
        from proteinshake.frameworks.tf import TensorflowVoxelDataset

        return TensorflowVoxelDataset(
            self.voxels, self.size, self.path + ".tf", *args, **kwargs
        )

    def np(self, *args, **kwargs):
        from proteinshake.frameworks.np import NumpyVoxelDataset

        return NumpyVoxelDataset(
            self.voxels, self.size, self.path + ".np", *args, **kwargs
        )
