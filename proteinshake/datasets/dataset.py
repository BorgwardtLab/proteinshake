...
class Dataset:
    ...
    def __init__(
        ...
        similarity_threshold_structure=0.9,
        similarity_threshold_sequence=0.9,
    ):
        self.repository_url = (
            f"https://sandbox.zenodo.org/record/{RELEASES[release]}/files"
        )
        self.n_jobs = n_jobs
    ...
    def proteins(self, resolution="residue"):
        ...
        def reader():
            with open(
                f"{self.root}/{self.name}.{resolution}.avro", "rb"
            ) as file:
    ...
    def check_arguments_same_as_hosted(self):
        ...
        if self.use_precomputed and not all(
            [v == getattr(self, k) for k, v in default_args.items()]
        ):
            print(
                "Error: The dataset arguments do not match the precomputed dataset arguments (the default settings). Set use_precomputed to False if you wish to generate a new dataset."
            )
            exit()
    ...
    def download_precomputed(self, resolution="residue"):
        ...
        if not os.path.exists(f"{self.root}/{self.name}.{resolution}.avro"):
            download_url(
                f"{self.repository_url}/{self.name}.{resolution}.avro.gz",
                f"{self.root}",
            )
            print("Unzipping...")
    ...
    def parse(self):
        ...
        residue_proteins = [
            {"protein": p["protein"], "residue": p["residue"]}
            for p in proteins
        ]
        atom_proteins = [
            {"protein": p["protein"], "atom": p["atom"]} for p in proteins
        ]
    ...
    def parse(self):
        ...
        paths = [
            path
            for path, protein in zip(paths, proteins)
    ...
    def validate(self, df):
        ...
        if self.check_sequence and not max(df["residue_number"]) == len(
            df["residue_number"].unique()
        ):
            return False
    ...
    def compute_clusters_sequence(self, proteins):
        ...
        for threshold in thresholds:
            ...
            clusters, reps = cdhit_wrapper(
                ids, sequences, sim_thresh=threshold, n_jobs=self.n_jobs
            )
    ...
    def compute_clusters_structure(self, proteins, paths):
        ...
        if self.n_jobs == 1:
            print(
                "Computing the TM scores with use_precompute = False is very slow. Consider increasing n_jobs."
            )

        paths = [
            unzip_file(p, remove=False) if p.endswith(".gz") else p
            for p in tqdm(paths, desc="Unzipping")
        ]
        ...
        output = Parallel(n_jobs=self.n_jobs)(
            delayed(tmalign_wrapper)(*pair)
            for pair in tqdm(todo, desc="Structure clustering")
        )
        ...
        for d in thresholds:
            clusterer = AgglomerativeClustering(
                n_clusters=None, distance_threshold=(1 - d)
            )
            clusterer.fit(DM)
            for i, p in enumerate(proteins):
                p["protein"][f"structure_cluster_{d}"] = int(
                    clusterer.labels_[i]
                )
