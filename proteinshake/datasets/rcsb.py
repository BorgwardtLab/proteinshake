...
class RCSBDataset(Dataset):
    ...
    def download(self):
        if self.n_jobs == 1:
            print(
                "Warning: Downloading an RCSB dataset with use_precompute = False is very slow. Consider increasing n_jobs."
            )
        total = None
        i = 0
        batch_size = 5000
        ...
        while total is None or total > i:
            ...
            r = requests.get(
                f"https://search.rcsb.org/rcsbsearch/v2/query?json={json.dumps(payload)}"
            )
            try:
                response_dict = json.loads(r.text)
                ids.extend(
                    [
                        x["identifier"].split("_")[0]
                        for x in response_dict["result_set"]
                    ]
                )
            except:
        ...
        failed = Parallel(n_jobs=n_jobs)(
            delayed(self.download_from_rcsb)(id)
    ...
    def download_from_rcsb(self, id):
        try:
            r = requests.get(
                f"https://data.rcsb.org/rest/v1/core/polymer_entity/{id}/1"
            )
            obj = json.loads(r.text)
            download_url(
                f"https://files.rcsb.org/download/{id}.pdb.gz",
