...
class ProteinLigandInterfaceDataset(Dataset):
    ...
    def download(self):
        download_url(
            f"https://pdbbind.oss-cn-hangzhou.aliyuncs.com/download/PDBbind_v{self.version}_refined.tar.gz",
            f"{self.root}/raw",
        )
        extract_tar(
            f"{self.root}/raw/PDBbind_v{self.version}_refined.tar.gz",
            f"{self.root}/raw",
        )
        os.rename(f"{self.root}/raw/refined-set", f"{self.root}/raw/files")

        self.index_data = parse_pdbbind_PL_index(
            osp.join(
                self.root,
                "raw",
                "files",
    ...
    def add_protein_attributes(self, protein):
        pocket = self.pdb2df(
            f'{self.root}/raw/files/{protein["protein"]["ID"]}/{protein["protein"]["ID"]}_pocket.pdb'
        )
        ...
        pocket_res = np.array(
            pocket.loc[pocket["atom_type"] == "CA"]["residue_number"].tolist()
        )
        protein_res = np.array(protein["residue"]["residue_number"]).reshape(
            -1, 1
        )

        pocket_atom = np.array(pocket["residue_number"].tolist())
        protein_atom = np.array(protein["atom"]["residue_number"]).reshape(
            -1, 1
        )
        ...
        is_site_res[(pocket_res == protein_res).sum(axis=1).nonzero()] = 1.0
        is_site_atom[(pocket_atom == protein_atom).sum(axis=1).nonzero()] = 1.0

        protein["residue"]["binding_site"] = list(
            map(int, is_site_res.squeeze())
        )
        protein["atom"]["binding_site"] = list(
            map(int, is_site_atom.squeeze())
        )
