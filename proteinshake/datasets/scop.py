...
class SCOPDataset(RCSBDataset):
    ...
    def download(self):
        # get the annots
        download_url(
            f"http://scop.mrc-lmb.cam.ac.uk/files/scop-cla-latest.txt",
            f"{self.root}/raw/scop.txt",
        )
        ...
        failed = Parallel(n_jobs=self.n_jobs)(
    ...
    def add_protein_attributes(self, protein):
        ...
        try:
            match = self.scop.loc[
                self.scop["FA-PDBID"] == protein["protein"]["ID"].upper()
            ].iloc[0]
        except KeyError:
            return None
        scop_classes = dict(
            [cla.split("=") for cla in match["SCOPCLA"].split(",")]
        )
