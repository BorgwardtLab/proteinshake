...
class FrameworkDataset:
    ...
    def __init__(
        ...
        if not os.path.exists(f"{path}/{size-1}.pkl"):
            ...
            for data_item in tqdm(data_list, desc="Converting", total=size):
                ...
                if not self.pre_filter is None and not self.pre_filter(
                    data, protein_dict
                ):
                    continue
        ...
        assert (
            original_repr == transforms_repr
        ), f"The pre_transform and/or pre_filter are not the same as when the dataset was created. If you want to change them, delete the folder at {path}"
    ...
    def __getitem__(self, idx):
        ...
        if idx > self.size - 1:
            raise StopIteration
        data, protein_dict = self.load_transform(
            *load(f"{self.path}/{idx}.pkl")
        )
