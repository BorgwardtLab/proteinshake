...
tqdm.__init__ = partialmethod(
    tqdm.__init__, mininterval=60 * 60
)  # once per hour

datasets = importlib.import_module("proteinshake.datasets")

parser = argparse.ArgumentParser(
    description="Script to generate all datasets for release."
)
parser.add_argument(
    "--path",
    type=str,
    help="Path to store the final dataset objects.",
    default="./release",
)
parser.add_argument(
    "--scratch",
    type=str,
    help="Path to scratch (if on cluster).",
    default=os.path.expandvars(f"$TMPDIR/proteinshake"),
)
parser.add_argument(
    "--tag",
    type=str,
    help="Release tag",
    default=datetime.now().strftime("%d%b%Y").upper(),
)
parser.add_argument("--njobs", type=int, help="Number of jobs.", default=10)
parser.add_argument(
    "--dataset",
    type=str,
    help="Name of the dataset class (case sensitive)",
    default="RCSBDataset",
)
parser.add_argument(
    "--organism",
    type=str,
    help="Organism (for AlphaFold datasets)",
    default="swissprot",
)
...
print("Copying...")
shutil.copyfile(
    f"{SCRATCH}/{DATASET}/{DATASET}.residue.avro.gz",
    f"{PATH}/{DATASET}.residue.avro.gz",
)
shutil.copyfile(
    f"{SCRATCH}/{DATASET}/{DATASET}.atom.avro.gz",
    f"{PATH}/{DATASET}.atom.avro.gz",
)
if clustering:
    shutil.copyfile(
        f"{SCRATCH}/{DATASET}/{DATASET}.cdhit.json.gz",
        f"{PATH}/{DATASET}.cdhit.json.gz",
    )
    shutil.copyfile(
        f"{SCRATCH}/{DATASET}/{DATASET}.tmalign.json.gz",
        f"{PATH}/{DATASET}.tmalign.json.gz",
    )
