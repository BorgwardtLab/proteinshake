...
class TestDownload(unittest.TestCase):
    ...
    def test_protein_ligand(self):
        with tempfile.TemporaryDirectory() as tmp:
            ds = ProteinLigandInterfaceDataset(root=tmp, use_precomputed=False, verbosity=2)
    ...
    def test_protein_protein(self):
        with tempfile.TemporaryDirectory() as tmp:
            ds = ProteinProteinInterfaceDataset(root=tmp, use_precomputed=False, verbosity=2)
    ...
    def test_rcsb(self, mock):
        with tempfile.TemporaryDirectory() as tmp:
            ds = RCSBDataset(root=tmp, use_precomputed=False, verbosity=2)
    ...
    def test_go(self):
        with tempfile.TemporaryDirectory() as tmp:
            ds = GeneOntologyDataset(root=tmp, use_precomputed=False, verbosity=2)
    ...
    def test_ec(self):
        with tempfile.TemporaryDirectory() as tmp:
            ds = EnzymeCommissionDataset(root=tmp, use_precomputed=False, verbosity=2)
    ...
    def test_pfam(self):
        with tempfile.TemporaryDirectory() as tmp:
            ds = ProteinFamilyDataset(root=tmp, use_precomputed=False, verbosity=2)

    @mock.patch.object(TMAlignDataset, 'download', download_mock)
    @mock.patch.object(TMAlignDataset, 'get_raw_files', get_raw_files_mock)
    def test_tm(self):
        with tempfile.TemporaryDirectory() as tmp:
            ds = TMAlignDataset(root=tmp, use_precomputed=False, verbosity=2)
    ...
    def test_af(self, mock):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            ds = AlphaFoldDataset(root=tmp, organism=organism, use_precomputed=False, verbosity=2)

    @mock.patch.object(SCOPDataset, 'download', scop_download_mock)
    @mock.patch.object(SCOPDataset, 'get_raw_files', get_raw_files_mock)
    def test_scop(self):
        with tempfile.TemporaryDirectory() as tmp:
            ds = SCOPDataset(root=tmp, use_precomputed=False, verbosity=2)
    ...
    def test_decoys(self):
        with tempfile.TemporaryDirectory() as tmp:
            ds = ProteinLigandDecoysDataset(root=tmp, use_precomputed=False, verbosity=2)
