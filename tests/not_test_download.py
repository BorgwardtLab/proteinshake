...
class TestDownload(unittest.TestCase):
    @mock.patch(
        "proteinshake.datasets.ProteinLigandInterfaceDataset.limit",
        return_value=5,
    )
    ...
    @mock.patch(
        "proteinshake.datasets.ProteinProteinInterfaceDataset.limit",
        return_value=5,
    )
    def test_protein_protein(self, mock):
        with tempfile.TemporaryDirectory() as tmp:
            ds = ProteinProteinInterfaceDataset(
                root=tmp, use_precomputed=False
            )
    ...
    @mock.patch(
        "proteinshake.datasets.GeneOntologyDataset.limit", return_value=5
    )
    ...
    @mock.patch(
        "proteinshake.datasets.EnzymeCommissionDataset.limit", return_value=5
    )
    def test_go(self, mock):
        with tempfile.TemporaryDirectory() as tmp:
